...
template <typename T>
bool BST<T>::search(Node* p, const T& key) {
    if ( !p ) {
        return false;
    }
    if (key < p->data) {
...
template <typename T>
void BST<T>::print_in_order(Node* p) {
    if ( p ) {
...
template <typename T>
size_t BST<T>::get_height() {
    return get_height(root);
}
// Private method to get the height of the BST recursively
template <typename T>
size_t BST<T>::get_height(Node* p) {
    if (!p) {
        return 0;
    }
    size_t left_height = get_height(p->left);
    size_t right_height = get_height(p->right);
